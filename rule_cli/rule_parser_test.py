...
from rule_parser import Keyword, Value, Context, IntValue, IntRange, RunTime
...
cmd.next_arg_is("cmd2").next_arg_is("cmd3", optional=True)

rangeValue = IntRange("range1", 10, 20, help_string="Range1 help")
value_node = cmd.get_path("my_cmd/cmd1/my_int_value")
value_node.next_arg_is("end_cmd")

value_node.next_arg_is("cmd3", help="Cmd2 help", value=rangeValue, optional=True).\
    next_is("cmd4", help="End at cmd4")

value_node.next_args_are([("cmd4", "Cmd4 help"),
                          ("cmd5", "Cmd5 help", rangeValue)])
...
my_context.register_command(cmd, discard_garbage_tokens=False,
                            handler=my_command_handler)

cmd = my_context.get_command("my_cmd")
cmd.print_commands()
